...
// MODULOS
const { evento , verificada , sabrina , cintia , mandy , victoria , craftg} = require('./lib/evento')
...
module.exports = kconfig = async (kill, message) => {
	...
	const testeNumber = config.teste
    ...
    try {
		...
		const isteste = testeNumber.includes(user)
		...
		case 'craudiaofc':
			if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
			await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/FXWte6yYsDOAaxKEPhAQfK', 'S2')
			break
			
		case 'supremacia':
			if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
			await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/DpdmbDh189Q78QCoti37kQ', 'S2')
			break
			
	   case 'papel':
    	    const papel = ["https://nekos.life/api/v2/img/wallpaper"];
    	    const papelc = papel[Math.floor(Math.random() * papel.length)];
			const batatagt = await axios.get(papelc)
			await kill.sendFileFromUrl(from, `${batatagt.data.url}`, ``, `pega aqui um papel brabo principalmente se for hentai s2`, id)
            break
		...
		case 'criador':
			await kill.sendFile(from, './lib/media/img/shuvi.jpg', 'shuvi.jpg')
			await kill.sendPtt(from, './lib/media/audio/dono.mp3', 'dono.mp3')
       ...
       case 'translate':
		...
		case 'merosplebeus':
			if (!isOwner) {
				...
				let hehe = `═✪〘 Olá! Todos plebeus que existem no grupo! 〙✪═\n═✪〘 Lorde feudal viminio informa: ${body.slice(10)} 〙✪═\n\n`
				for (let i = 0; i < groupMem.length; i++) {
					hehe += '- '
					hehe += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
				}
				hehe += '\n═✪〘 Lorde feudal viminio deseja uma boa semana 〙✪═'
				await sleep(2000)
       ...
       case 'clima':
		...
		case 'reigado':
			if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
            await kill.sendFile(from, './lib/media/img/reigado.mp4', 'shuvi.mp4')
			await kill.sendTextWithMentions(from, `═✪〘O GRANDE REI DO GADO〙✪═ \n\n @${randomMember.replace(/@c.us/g, '')}\n\n═✪〘O REI DO GADO〙✪═`)
            await sleep(2000)
		break
        ...
        case 'ihentai':
		 ...
		 case 'ihentai2':
			if (isGroupMsg && !isNsfw) return await kill.reply(from, mess.gpadulto(), id)
    	    const hnntai = ["https://nekos.life/api/v2/img/hentai", "https://nekos.life/api/v2/img/pussy", "https://nekos.life/api/v2/img/pussy_jpg", "https://nekos.life/api/v2/img/classic", "https://api.computerfreaker.cf/v1/hentai"];
    	    const hentccv = hnntai[Math.floor(Math.random() * hnntai.length)];
			const hentai1l = await axios.get(hentccv)
			await axios.get(hentai1l.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				const hntimg = Buffer.from(response.data, 'binary').toString('base64')
			})
            break
		...
		case 'cr+18':
			await kill.sendFile(from, './lib/media/img/evento.jpg', 'evento.jpg', evento, id)
			break
		
		case 'verificadas':
			await kill.sendFile(from, './lib/media/img/evento.jpg', 'evento.jpg', verificada, id)
			break
			
		case 'duda':
		await kill.sendFile(from, './lib/media/img/duda2.jpg', 'shuvi.jpg')
	   await kill.reply(from, ' Videos de 10 minutos avulsos por 15 , 10 fotos e dois mini videos por 20$ , 15 fotos e 5 mini videos por 25$ , Pack 7 videos por 20,00  FALAR COM 5511985350484@c.us', id)
			break
			
		case 'verificar':
			await kill.reply(from, 'de .criador e fale pra se verificar pra poder vender pack ', id)
			break
			
			
		case 'gueixa':
		await kill.sendFile(from, './lib/media/img/gueixa.jpg', 'shuvi.jpg')
	   await kill.reply(from, 'batata', id)
			break
			
					
		case 'sabrina':
		await kill.sendFile(from, './lib/media/img/sabrina.jpg', 'sabrinapack.jpg', sabrina, id)
			break
			
		case 'cintia':
		await kill.sendFile(from, './lib/media/img/cintia.jpg', 'cintiapack.jpg', cintia, id)	   
			break
			
		case 'mandy':
		await kill.sendFile(from, './lib/media/img/mandy.jpg', 'cintiapack.jpg', mandy, id)	   
			break
			
		case 'victoria':
		await kill.sendFile(from, './lib/media/img/victoria.jpg', 'cintiapack.jpg', victoria, id)	   
			break
			
		case 'ghentai':
		await kill.sendFile(from, './lib/media/img/hentai.jpg', 'shuvi.jpg')
		await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/HHHwCcuDiPgILNeYbaw2cH', 'S2')
		break


		case 'gporno':
		await kill.sendFile(from, './lib/media/img/hentai.jpg', 'shuvi.jpg')
		await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/CAdA6RtXkrsGUyYogEWFzl', 'S2')
		break
		
		case 'shuvi':
				const shuvi = fs.readFileSync('./lib/config/shuvi.json')
				const shuvii = JSON.parse(shuvi)
				const shuvis = Math.floor(Math.random() * shuvii.length)
				const shuviis = shuvii[shuvis]
				await kill.sendFileFromUrl(from, shuviis.image, 'waifu.jpg', shuviis.teks, id)
            break
		...
		case 'craftgx':
		await kill.sendFile(from, './lib/media/img/craftg.jpg', 'craftg.jpg', craftg, id)	   
			break
		...
		case 'otaku':
			...
			case 'ungar':
		await kill.sendFile(from, './lib/media/img/testee.mp4', 'shuvi.mp4')
			break
			
			case 'gura':
		await kill.sendFile(from, './lib/media/img/gura.mp4', 'shuvi.mp4')
			break
        ...
        case 'fig':
            ...
            if (isMedia && isImage) {
				...
				await kill.sendImageAsSticker(from, imageBase64, { author: 'batata', pack: 'craudia', keepScale: true })
            } else if (isQuotedImage) {
                const mediaData = await decryptMedia(quotedMsg, uaOverride)
                ...
                const url = args[0]
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: 'batata', pack: 'craudia', keepScale: true })
